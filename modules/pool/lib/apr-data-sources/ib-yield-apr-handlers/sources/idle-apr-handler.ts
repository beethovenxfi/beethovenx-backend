...
class IdleAprHandler implements AprHandler {
    ...
    async getAprs() {
        try {
            const aprPromises = Object.entries(this.idleTokens).map(async ([tokenName, idleTokenAddress]) => {
                const { data } = await axios.get(
                    [this.baseIdleApiUrl, idleTokenAddress, '?isRisk=false&order=desc&limit=1'].join(''),
                    {
                        headers: {
                ...
                return [this.wrappedIdleTokens[tokenName], value];
            });
